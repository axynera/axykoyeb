...
if (!process.env.OAUTH_SIGNING_SECRET && !process.env.SESSION_SECRET) {
  console.warn('[SECURITY] Set OAUTH_SIGNING_SECRET in Koyeb so OAuth credentials survive restarts.');
}
...
if (process.env.GITHUB_CLIENT_ID && process.env.GITHUB_CLIENT_SECRET) {
  passport.use(new GitHubStrategy({
    ...
    if (!dynamicOwnerUsername) {
      ...
      console.log(`[AUTH] Owner di-set ke: ${username}`);
    }
...
function verifyObject(token, expectedType) {
  try {
...
function clientFromId(clientId, clientSecret) {
  if (!clientId || !clientSecret) return null;
  return safeEqual(clientSecret, clientSecretFor(clientId)) ? { clientId, clientSecret } : null;
}
...
app.post('/oauth/register', (req, res) => {
  ...
  const clientName = req.body.client_name || 'ChatGPT Terminal Connector';
...
function getClient(req) {
  ...
  return clientFromId(clientId, clientSecret);
}
...
app.get('/oauth/auth', (req, res) => {
  ...
  if (!client_id || !redirect_uri) return res.status(400).send('client_id dan redirect_uri diperlukan.');
  if (clientSecretFor(client_id) === `secret_${hmac(`client:${client_id}`)}` === false) return res.status(400).send('Client OAuth tidak dikenal.');
...
app.post('/oauth/login', (req, res) => {
  ...
  if (!client_id || !redirect_uri || code_challenge_method !== 'S256' || !code_challenge) return res.status(400).send('Permintaan OAuth tidak valid.');
  ...
  const code = signObject({ typ: 'code', cid: client_id, ru: redirect_uri, cc: code_challenge, sc: scope || 'terminal', exp: Math.floor(Date.now() / 1000) + 300, jti: randomId(12) });
...
app.post('/oauth/token', (req, res) => {
  ...
  if (grantType === 'authorization_code') {
    ...
    if (!record || record.cid !== client.clientId || record.ru !== req.body.redirect_uri) return res.status(400).json({ error: 'invalid_grant' });
...
function requireBearer(req, res, next) {
  ...
  if (!header.startsWith('Bearer ')) {
    res.set('WWW-Authenticate', `Bearer realm="mcp", resource_metadata="${baseUrl(req)}/.well-known/oauth-protected-resource"`);
...
function executeShellCommand(command, cwd, timeoutMs) {
  return new Promise(resolve => {
    ...
    const args = os.platform() === 'win32' ? ['-NoLogo', '-NoProfile', '-NonInteractive', '-Command', command] : ['-lc', command];
    const startedAt = Date.now();
    const child = execFile(shell, args, { cwd: cwd || getDefaultCwd(), env: process.env, timeout: Math.min(Math.max(Number(timeoutMs) || MCP_EXEC_TIMEOUT_MS, 1000), 300000), maxBuffer: 10 * 1024 * 1024 }, (error, stdout, stderr) => {
      resolve({ exitCode: typeof error?.code === 'number' ? error.code : (error ? 1 : 0), stdout: String(stdout || ''), stderr: String(stderr || ''), duration_ms: Date.now() - startedAt, timedOut: error?.killed === true });
    });
    child.on('error', error => resolve({ exitCode: 1, stdout: '', stderr: String(error.message || error), duration_ms: Date.now() - startedAt, timedOut: false }));
  });
}
...
async function handleToolCall(name, args = {}) {
  ...
  if (name === 'terminal_env') {
    const safeKeys = ['NODE_ENV','PORT','HOME','HOSTNAME','PWD','SHELL','USER','LANG','TZ'];
    const environment = Object.fromEntries(safeKeys.filter(k => Object.prototype.hasOwnProperty.call(process.env, k)).map(k => [k, process.env[k]]));
    Object.assign(environment, { platform: process.platform, arch: process.arch, node: process.version });
    return { content: [{ type: 'text', text: JSON.stringify(environment, null, 2) }], structuredContent: { environment } };
  }
  if (name === 'terminal_exec') {
    if (typeof args.command !== 'string' || !args.command.trim()) return { isError: true, content: [{ type: 'text', text: 'Parameter "command" wajib diisi.' }] };
    const result = await executeShellCommand(args.command, args.cwd || getDefaultCwd(), args.timeout_ms);
    const output = [`exit_code: ${result.exitCode}`, `duration_ms: ${result.duration_ms}`, result.timedOut ? 'timed_out: true' : '', '', '--- stdout ---', result.stdout || '(empty)', '', '--- stderr ---', result.stderr || '(empty)'].filter(Boolean).join('\n');
...
async function handleMcpMessage(message, req) {
  ...
  if (method === 'initialize') return jsonRpcResult(id, { protocolVersion: params.protocolVersion === MCP_PROTOCOL_LEGACY ? MCP_PROTOCOL_LEGACY : MCP_PROTOCOL_LEGACY, capabilities: { tools: { listChanged: false } }, serverInfo: { name: SERVER_NAME, version: SERVER_VERSION }, instructions: 'Koyeb Web Terminal MCP menyediakan terminal_exec, terminal_pwd, dan terminal_env.' });
  if (method === 'notifications/initialized') return null;
  if (method === 'ping') return jsonRpcResult(id, {});
  if (method === 'tools/list') { console.log('[MCP:TOOLS_LIST]'); return jsonRpcResult(id, { tools: MCP_TOOLS }); }
  if (method === 'tools/call') {
    console.log(`[MCP:TOOLS_CALL] ${params.name || 'unknown'}`);
    if (!MCP_TOOLS.some(t => t.name === params.name)) return jsonRpcError(id, -32602, `Unknown tool: ${params.name}`);
    try { return jsonRpcResult(id, await handleToolCall(params.name, params.arguments || {})); } catch (error) { console.error('[MCP:TOOLS_CALL] error', error); return jsonRpcError(id, -32603, 'Internal error', { message: String(error?.message || error) }); }
  }
...
async function handleMcpRequest(req, res) {
  ...
  if (Array.isArray(body)) {
    const responses = [];
    for (const message of body) { const response = await handleMcpMessage(message, req); if (response !== null) responses.push(response); }
    if (!responses.length) return res.status(202).end();
    return sendMcpJson(res, responses);
  }
  const response = await handleMcpMessage(body, req);
  if (response === null) return res.status(202).end();
  return sendMcpJson(res, response);
}

function sendMcpJson(res, payload) {
  res.status(200).set({ 'Content-Type': 'application/json; charset=utf-8', 'Cache-Control': 'no-store', 'MCP-Protocol-Version': MCP_PROTOCOL_CURRENT }).json(payload);
}

function validateMcpRequest(req) {
  const headerMethod = req.get('Mcp-Method');
  const bodyMethod = req.body?.method;
  if (headerMethod && bodyMethod && headerMethod !== bodyMethod) return false;
  return true;
}

app.post('/mcp', requireBearer, async (req, res) => {
  if (!validateMcpRequest(req)) return res.status(400).json(jsonRpcError(req.body?.id ?? null, -32600, 'Mcp-Method header does not match JSON-RPC method'));
  ...
  console.log(`[MCP] POST ${req.body?.method || 'unknown'}`);
...
app.get('/mcp/sse', requireBearer, (req, res) => {
  res.set({ 'Content-Type': 'text/event-stream; charset=utf-8', 'Cache-Control': 'no-cache, no-store', Connection: 'keep-alive', 'X-Accel-Buffering': 'no' });
  res.flushHeaders();
...
io.on('connection', socket => {
  console.log('[TERMINAL] WebSocket terminal connected');
  const ptyProcess = pty.spawn(shell, [], { name: 'xterm-color', cols: 80, rows: 30, cwd: getDefaultCwd(), env: process.env });
  ptyProcess.onData(data => socket.emit('output', data));
  socket.on('input', data => { if (typeof data === 'string') ptyProcess.write(data); });
  socket.on('resize', size => { if (!size) return; const cols = Math.max(1, Math.min(500, Number(size.cols) || 80)); const rows = Math.max(1, Math.min(500, Number(size.rows) || 30)); ptyProcess.resize(cols, rows); });
  socket.on('disconnect', () => { try { ptyProcess.kill(); } catch (_) {} });
});

app.use((err, req, res, next) => {
  console.error('[HTTP ERROR]', err);
  if (res.headersSent) return next(err);
  res.status(500).json({ error: 'internal_server_error', message: process.env.NODE_ENV === 'production' ? 'Internal server error' : String(err?.message || err) });
});

function startUptimeBots() {
  const appUrl = process.env.APP_URL || process.env.KOYEB_PUBLIC_URL || process.env.RENDER_EXTERNAL_URL || `http://localhost:${PORT}`;
  for (const minutes of [3, 4, 5]) setInterval(() => {
    const target = `${appUrl.includes('http') ? appUrl.replace(/\/$/, '') : `https://${appUrl}`}/ping`;
    const client = target.startsWith('https') ? https : http;
    client.get(target, response => { console.log(`[Uptime Bot ${minutes}m] Ping status: ${response.statusCode}`); response.resume(); }).on('error', () => {});
  }, minutes * 60 * 1000);
...
server.listen(PORT, '0.0.0.0', () => {
  ...
  console.log('✅ OAuth: stateless + PKCE');
  console.log(`✅ MCP: /mcp (${MCP_PROTOCOL_CURRENT})`);
