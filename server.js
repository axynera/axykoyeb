...
app.post('/oauth/register', (req, res) => {
  ...
  oauthClients.set(clientId, { clientId, clientSecret, redirectUris, clientName: req.body.client_name || 'ChatGPT Terminal Connector' });
  console.log(`[DCR] Registered client: ${clientId}`);
  res.status(201).json({ client_id: clientId, client_secret: clientSecret, client_name: req.body.client_name || 'ChatGPT Terminal Connector', redirect_uris: redirectUris, token_endpoint_auth_method: 'client_secret_post', grant_types: ['authorization_code', 'refresh_token'], response_types: ['code'] });
});
...
app.post('/oauth/login', (req, res) => {
  ...
  const client = oauthClients.get(client_id);
  const configuredPassword = process.env.TERMINAL_PASSWORD || process.env.SESSION_SECRET;
